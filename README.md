...
截至 2026-09-13，已上线 Chapter 1 → Unit 1 → Part 1：Waking Up & Being Late。

- 六模块：Vocabulary、Scenes、Expressions、Don't Mix These Up、Useful Lines、Practice。
- 41 个词汇条目默认折叠，支持单条展开及标题旁“全部展开 / 全部收起”；17 个条目附美式音标。
- 方案 C 多色系高雅视觉系统（海港暮蓝 + 珊瑚暖红 + 雅致橄榄金），支持 ☀️ 日间淡雅 / 🌙 夜间护眼 模式一键切换（`#themeToggle`）。
- 单词发音按钮升级为 16px 矢量 SVG 扬声器图标；闲置态为 Slate 灰静音底调，播放中激活珊瑚暖红微光呼吸动效。
- 侧边栏与右侧浮动目录（`.lesson-directory`）当前模块配备 **3.5px 珊瑚暖红指示条**与浅暖底色高亮。
- 四张高清教学图，图内编号、图下双语表达与点读；主图最大 520px，场景图最大 440px，可点击查看大图。
- 桌面右侧浮动目录（跟随页面实时滚动定位），窄屏顶部粘性工具栏提供紧凑入口；另有课程层级导航、搜索和顶部/底部按钮。
